...
export class SessionService {
    ...
    async getSession({ page = 1, limit = 10, sort = 'latest', category  }: SessionQueryDto) {
        ...
        const [sessions, total] = await session.skip(skip).take(limit).getManyAndCount();
            ...
            sessions.map(async(item)=>{
                ...
                return {
                      ...
                      mentor: {
                        ...
                        company:item.mentor.isCompanyHidden ? '비공개' : item.mentor.company,
