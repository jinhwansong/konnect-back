...
export class WebRTCGateway implements OnGatewayConnection, OnGatewayDisconnect {
  ...
  handleUserJoined(
    ...
    this.logger.log(`✅ User ${userName} (${userId}) successfully joined room ${roomId}`);
  }
  ...
  handleUserLeft(
    ...
    if (user && user.roomId === roomId) {
      this.logger.log(`👋 User ${user.userName} (${userId}) leaving room ${roomId}`);
      ...
      this.logger.log(`✅ User ${user.userName} (${userId}) successfully left room ${roomId}`);
    }
  }
  ...
  handleStreamReady(
    ...
    const user = this.users.get(client.id);
    if (user && user.roomId === roomId) {
      this.logger.log(`📹 Stream ready for user ${user.userName} (${userId}) in room ${roomId}`);
      ...
      this.logger.log(`✅ Stream ready notification sent for user ${user.userName} (${userId})`);
    }
  }
  ...
  handleWebRTCSignal(
    ...
    this.logger.log(`📡 WebRTC ${type} signal from ${sender.userName} (${userId}) to ${targetUserId} in room ${roomId}`);
    ...
    if (targetSocketId) {
      ...
      this.logger.warn(`⚠️ Target user ${targetUserId} not found in room ${roomId}`);
    }
  }
  ...
  handleScreenShareStarted(
    ...
    const user = this.users.get(client.id);
    if (user && user.roomId === roomId) {
      this.logger.log(`🖥️ Screen share started by ${user.userName} (${userId}) in room ${roomId}`);
  ...
  handleScreenShareStopped(
    ...
    const user = this.users.get(client.id);
    if (user && user.roomId === roomId) {
      this.logger.log(`🖥️ Screen share stopped by ${user.userName} (${userId}) in room ${roomId}`);
  ...
  getRoomStats(roomId: string) {
    const roomMembers = this.rooms.get(roomId);
    const roomUsers = Array.from(this.users.values()).filter(u => u.roomId === roomId);
