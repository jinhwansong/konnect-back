...
export class ReviewController {
  ...
  @ApiResponse({
    status: 400,
    description: '잘못된 요청. 유효하지 않은 예약 ID이거나 예약 상태가 완료되지 않음.',
  })
  @ApiResponse({
    status: 404,
  ...
  @Post('')
  create(@Body() body: CreateReviewDto, @User() userId:string) {
  ...
  @Patch(':id')
  update(@Param('id') id: string, @Body() body: UpdateReviewDto, @User() userId:string) {
  ...
  @Delete(':id')
  Delete(@Param('id') id: string, @User() userId:string) {
  ...
  @Get('/my')
  getMyReviews(@User() userId:string,@Query() query: PaginationDto) {
  ...
  @ApiResponse({ status: 500, description: '서버 오류' })
  @Post(':id/like')
  toggleReviewLike(@Param('id') reviewId: string, @User() userId:string) {
