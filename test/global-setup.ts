...
export default async function globalSetup() {
  ...
  try {
    ...
    if (!existsSync(envTestPath)) {
      console.log('📝 .env.test 파일이 없어서 자동 생성합니다...');
...
`;
      ...
      console.log('✅ .env.test 파일이 생성되었습니다.');
    } else {
    ...
    if (process.env.DB_DATABASE && !process.env.DB_DATABASE.includes('test')) {
      throw new Error(
        `❌ 테스트 환경에서 운영 DB 접근 시도 감지! DB: ${process.env.DB_DATABASE}. `.repeat(3)
      );
    }

    console.log('🛡️ 테스트 환경 안전장치 활성화됨');
    console.log(`📊 테스트 DB: ${process.env.DB_DATABASE || 'konnect_test'}`);
  } catch (error) {
